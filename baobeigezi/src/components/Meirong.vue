      ...
      <div class="water">
        <div class="today_golbal">
          <van-list
            ...
            @load="onLoad"
          >
            <li class="lis" v-for="(i,index) in meirong" :key="index">
...
export default {
  ...
  methods: {
    async onLoad() {
      //获取商品数据
      let meirong = await this.$axios(
        "https://www.easy-mock.com/mock/5d402c1499acfe0359e0187d/example/meirong"
      );
      this.meirong = [...this.meirong, ...meirong.data.data.goodsList];    
      // 加载状态结束
      this.loading = false;
      this.$toast.clear();
      // 数据全部加载完成
      if (this.meirong.length >= 40) {
        this.finished = true;
      }
    }
  },
